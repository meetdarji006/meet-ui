...
export function CodeBlock({ code, language = "tsx", showLineNumbers = true }: CodeBlockProps) {
    ...
    return (
        <div className="relative group rounded-2xl overflow-hidden border border-white/6">
            {/* Header bar */}
            <div className="flex items-center justify-between px-5 py-3 bg-white/2 border-b border-white/6">
                <div className="flex items-center gap-2.5">
                    {/* Traffic lights */}
                    <div className="flex items-center gap-1.5 mr-2">
                        <span className="w-3 h-3 rounded-full bg-white/6 border border-white/8" />
                        <span className="w-3 h-3 rounded-full bg-white/6 border border-white/8" />
                        <span className="w-3 h-3 rounded-full bg-white/6 border border-white/8" />
                    </div>
                ...
                <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${copied
                        ...
                        : 'bg-white/3 border-white/8 hover:bg-white/6 hover:border-white/12 text-neutral-400 hover:text-white'
                        }`}
                >
                    {copied ? (
                        <>
            ...
            <div className="relative bg-[#06000e]">
                {/* Subtle glow accents */}
                <div className="absolute top-0 left-0 w-48 h-48 bg-purple-500/2 blur-3xl pointer-events-none" />
                <div className="absolute bottom-0 right-0 w-48 h-48 bg-indigo-500/2 blur-3xl pointer-events-none" />

                <pre className="relative p-5 overflow-x-auto text-[13px] font-mono leading-7 max-h-[500px]">
                    <code className="block">
                        {lines.map((line, i) => (
                            <div key={i} className="flex hover:bg-white/2 -mx-5 px-5 rounded">
                                {showLineNumbers && (
