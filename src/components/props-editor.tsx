...
import { PropConfig } from "@/lib/components-data/registry"
...
export function PropsEditor({ props, values, onChange }: PropsEditorProps) {
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            {props.map((prop) => (
                <div key={prop.name} className="space-y-1.5 sm:space-y-2">
                    <label className="text-[10px] sm:text-xs font-mono font-medium text-neutral-500 uppercase tracking-wider">
                        {prop.name}
                    </label>
