// Auto-generated from aurora-cursor.tsx
// Run: npm run generate-codes

export const auroraCursorCodeTS = `"use client"
...
export const AuroraCursor = ({
    ...
    }, [enabled, smoothing, isVisible, isMobile])
...
export const auroraCursorCodeJS = `"use client";
import { useEffect, useRef, useState } from "react";
import { motion } from "framer-motion";
export const AuroraCursor = ({ size = 100, smoothing = 0.12, enabled = true, }) => {
    const [position, setPosition] = useState({ x: -500, y: -500 });
    const [isVisible, setIsVisible] = useState(false);
    const targetRef = useRef({ x: -500, y: -500 });
    const currentRef = useRef({ x: -500, y: -500 });
    ...
    return (<div style={{
            ...
            {layers.map((layer, index) => (<motion.div key={index} initial={{ opacity: 0, scale: 0.5 }} animate={{
                opacity: isVisible ? 0.6 - index * 0.1 : 0,
                scale: isVisible ? layer.scale : 0.5,
                rotate: [layer.rotation, layer.rotation + 360],
            }} transition={{
