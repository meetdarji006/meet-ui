// Auto-generated from luminous-card.tsx
// Run: npm run generate-codes

export const luminousCardCodeTS = `"use client"
...
export const luminousCardCodeJS = `"use client";
import React from "react";
import { cn } from "@/lib/utils";
const DefaultIcon = ({ color }) => (<svg xmlns="http://www.w3.org/2000/svg" width="2.4em" height="2.4em" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
    </svg>);
export function LuminousCard({ title = "Luminous Design", description = "Light Folds Around Form\\nRevealing Layers Of Depth", icon, accentColor = "#a78bfa", bgColor = "#1a1028", intensity = 0.7, showFrame = true, showIcon = true, titleClass, descriptionClass, className, }) {
    const uid = React.useId().replace(/:/g, "");
    return (<div className={cn(\`lc-\${uid}\`, "relative flex items-center justify-center", className)}>
            <style>{\`
                .lc-\${uid} {
                    --accent: \${accentColor};
                    --bg: \${bgColor};
            ...
            <div className="lc-card">
                ...
                {showIcon && (<div className="lc-icon">
                        {icon || <DefaultIcon color={accentColor}/>}
                    </div>)}
