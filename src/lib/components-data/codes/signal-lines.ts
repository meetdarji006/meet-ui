// Auto-generated from signal-lines.tsx
// Run: npm run generate-codes

export const signalLinesCodeTS = `"use client"
...
export const signalLinesCodeJS = `"use client";
import React, { useRef, useMemo, useEffect } from "react";
import { Canvas, useFrame, useThree } from "@react-three/fiber";
import * as THREE from "three";
import { EffectComposer, Bloom } from "@react-three/postprocessing";
...
const MAX_TRAIL = 150;
// --- PATH CALCULATION ---
function getPathPoint(t, lineIndex, lineCount, time, options) {
    const totalLen = options.curveLength + options.straightLength;
    const currentX = -options.curveLength + t * totalLen;
    let y = 0;
    let z = 0;
...
function Signals({ options }) {
    ...
    const pickSignalColor = () => {
        ...
        if (options.useColor2)
            choices.push(signalColors.color2);
        if (options.useColor3)
            choices.push(signalColors.color3);
        return choices[Math.floor(Math.random() * choices.length)].clone();
    };
    useEffect(() => {
        if (!groupRef.current)
...
function SceneContent({ options }) {
    ...
    return (<group ref={groupRef} position={[posX, options.positionY, 0]} rotation={[0, 0, THREE.MathUtils.degToRad(options.globalRotation)]}>
            <BackgroundLines options={options}/>
            <Signals options={options}/>
        </group>);
}
export default function SignalLines({ className, height = "100%", width = "100%", options = {} }) {
    ...
    return (<div className={className} style={{
            ...
            <Canvas key={\`canvas-\${mergedOptions.transparent}\`} style={{ width: '100%', height: '100%', background: 'transparent' }} camera={{ position: [0, 0, 90], fov: 45 }} gl={{
            antialias: true,
            alpha: true,
            premultipliedAlpha: false,
            preserveDrawingBuffer: true
                ...
                <SceneContent options={mergedOptions}/>
                {/* Skip bloom when transparent to avoid opaque render targets */}
                {!mergedOptions.transparent && (<EffectComposer>
                        <Bloom intensity={mergedOptions.bloomStrength} luminanceThreshold={0} luminanceSmoothing={0.9} radius={mergedOptions.bloomRadius}/>
                    </EffectComposer>)}
            </Canvas>
        </div>);
}
