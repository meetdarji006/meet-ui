// Auto-generated from magnetic-cursor.tsx
// Run: npm run generate-codes

export const magneticCursorCodeTS = `"use client";
...
export const MagneticCursor = ({
    ...
    }, [enabled, circleCount, smoothing, isMobile]);
...
export const magneticCursorCodeJS = `"use client";
import { useEffect, useRef, useState } from "react";
export const MagneticCursor = ({ circleCount = 5, circleSize = 40, color = "rgba(255, 255, 255, 0.6)", smoothing = 0.15, borderWidth = 1, enabled = true, hoverScale = 1.5, interactiveSelector = "a, button, [role='button'], input, textarea, [data-magnetic-hover]", }) => {
    const [positions, setPositions] = useState(Array(circleCount).fill({ x: 0, y: 0 }));
    const [isHovering, setIsHovering] = useState(false);
    const [isVisible, setIsVisible] = useState(false);
