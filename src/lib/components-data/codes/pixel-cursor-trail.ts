// Auto-generated from pixel-cursor-trail.tsx
// Run: npm run generate-codes

export const pixelCursorTrailCodeTS = `"use client";
...
export function PixelCursorTrail({
    ...
    const handleMouseMove = useCallback(
        ...
        [createPixel, pixelSize, trailLength, isGlobal, isMobile]
    );
...
export const pixelCursorTrailCodeJS = `"use client";
import React, { useEffect, useRef, useState, useCallback } from "react";
import { cn } from "@/lib/utils";
export function PixelCursorTrail({ pixelSize = 12, trailLength = 40, fadeSpeed = 0.04, pixelColor, isGlobal = false, className, ...props }) {
    ...
    if (isMobile)
        return null;
    return (<div ref={containerRef} onMouseMove={!isGlobal ? handleMouseMove : undefined} className={cn(isGlobal
            ? "fixed inset-0 pointer-events-none z-9999 overflow-hidden"
            : "relative w-full h-full min-h-[400px] overflow-hidden cursor-crosshair select-none bg-background", className)} {...props}>
            {pixels.map((pixel) => {
            // Calculate size based on age - older pixels are smaller
            const sizeMultiplier = Math.max(0.3, 1 - pixel.age / 100);
            const currentSize = pixelSize * sizeMultiplier;
            return (<div key={pixel.id} className={cn("absolute pointer-events-none", !pixelColor && "bg-foreground")} style={{
                    left: pixel.x - currentSize / 2,
                    top: pixel.y - currentSize / 2,
                    width: currentSize,
                    height: currentSize,
            ...
            {!isGlobal && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <span className="text-xs font-mono uppercase tracking-[0.3em] text-foreground/20">Move cursor</span>
                </div>)}
        </div>);
}
