...
export function HoverNameGallery({ className, items = defaultItems, defaultTitle = "DIRECTORS" }) {
    ...
    const activeItem = items.find(item => item.id === hoveredId);
    const currentText = activeItem ? activeItem.name : defaultTitle;
    return (<div className={cn("relative w-full min-h-[350px] sm:min-h-[420px] lg:min-h-[500px] flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8", className)}>

            {/* Gallery Row */}
            <div className="flex items-center justify-center gap-1 sm:gap-1.5 lg:gap-2 mb-8 sm:mb-12 lg:mb-16 h-[130px] sm:h-[160px] lg:h-[200px]" onMouseLeave={() => setHoveredId(null)}>
                {items.map((item) => {
            const isHovered = hoveredId === item.id;
            const isOtherHovered = hoveredId !== null && hoveredId !== item.id;
            ...
            <div className="relative w-full max-w-7xl h-[120px] sm:h-[180px] lg:h-[250px] flex items-center justify-center overflow-hidden">
                <AnimatePresence mode="popLayout">
                    <motion.div key={currentText} className={cn("absolute flex items-center justify-center text-[14vw] sm:text-[10vw] lg:text-[180px] leading-none font-black tracking-tighter uppercase scale-y-[1.5]", hoveredId ? "text-purple-500" : "text-white")} style={{
            fontFamily: "Impact, Arial Black, sans-serif"
        }}>
                        {currentText.split("").map((char, i) => {
            const centerIndex = currentText.length / 2;
