// Auto-generated from zajno-text-hover.tsx
// Run: npm run generate-codes

export const zajnoTextHoverCodeTS = `"use client"
...
export const zajnoTextHoverCodeJS = `"use client";
import React, { useRef, useState, useEffect } from "react";
import { motion, useMotionValue, useSpring, useTransform } from "framer-motion";
import { cn } from "@/lib/utils";
export const ZajnoTextHover = ({ text, slices = 20, className, textClassName, displacement = 40, displacementX = 10, activationDistance = 150, }) => {
    ...
    // Pre-calculate slice percentages
    const sliceWidthPercent = 100 / slices;
    return (<div ref={containerRef} className={cn("relative inline-block overflow-hidden", className)} onMouseMove={handleMouseMove} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} style={{ cursor: "default" }}>
            {/* Base Text (Invisible, just for sizing the container properly) */}
            <span className={cn("invisible opacity-0 select-none", textClassName)}>
                {text}
            </span>
            ...
            // For each slice, calculate its center X position percentage
            const sliceCenterXPercent = (i + 0.5) * sliceWidthPercent;
            return (<Slice key={i} index={i} text={text} textClassName={textClassName} sliceWidthPercent={sliceWidthPercent} sliceCenterXPercent={sliceCenterXPercent} smoothMouseX={smoothMouseX} isHovered={isHovered} displacement={displacement} displacementX={displacementX} activationDistance={activationDistance} containerRef={containerRef}/>);
        })}
            </div>
        </div>);
};
// Inner component for each vertical slice to compute its own transform
const Slice = ({ index, text, textClassName, sliceWidthPercent, sliceCenterXPercent, smoothMouseX, isHovered, displacement, displacementX, activationDistance, containerRef }) => {
    const [sliceCenterX, setSliceCenterX] = useState(0);
    useEffect(() => {
        // Calculate the absolute pixel center of this slice for distance math
        const calculateCenter = () => {
    ...
    // Calculate how far this specific slice is from the mouse pointer dynamically
    const distanceToMouse = useTransform(smoothMouseX, (currentMouseX) => {
        if (!isHovered)
            return activationDistance; // Force it to act like it's outside the radius when not hovered
        return Math.abs(currentMouseX - sliceCenterX);
    });
    // Spring the distance so that when isHovered changes, the text gracefully returns to center
    const smoothDistance = useSpring(distanceToMouse, { damping: 40, stiffness: 150 });
    const yTransform = useTransform(smoothDistance, (dist) => {
        // If the mouse is within the activation radius, displace it!
        if (dist < activationDistance) {
            // Normalized distance from 0 to 1 (0 is exactly on cursor, 1 is at the edge of activation)
            const normalizedDistance = dist / activationDistance;
    ...
    const xTransform = useTransform(smoothDistance, (dist) => {
        if (dist < activationDistance) {
            const normalizedDistance = dist / activationDistance;
            const intensity = 1 - normalizedDistance;
            // Alternate horizontal direction differently than vertical (e.g. based on index modulo 3) to add chaos
    ...
    return (<div className="h-full overflow-hidden relative" style={{ width: \`\${sliceWidthPercent}%\` }}>
            <motion.div style={{ y: yTransform, x: xTransform }} className="absolute inset-0 flex items-center justify-center whitespace-nowrap">
                <span className={cn("absolute transform origin-left whitespace-nowrap", textClassName)} style={{
            // The critical trick: Offset the inner text to the left by exactly this slice's horizontal starting position
            // This makes the text segment inside the hidden overflow align perfectly as if it was one continuous string
            left: \`-\${index * 100}%\`
        }}>
