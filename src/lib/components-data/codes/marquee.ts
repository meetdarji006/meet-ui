// Auto-generated from marquee.tsx
...
import { cn } from "@/lib/utils";
export default function Marquee({ children, speed = 40, gap = 40, direction = "left", pauseOnHover = true, fade = true, className, }) {
    // Calculate animation duration based on speed — higher speed = shorter duration
    const duration = Math.max(400 / speed, 2);
    const content = children ?? <DefaultMarqueeItems />;
    return (<div className={cn("group relative flex w-full overflow-hidden", className)} style={fade ? {
            maskImage: "linear-gradient(to right, transparent, black 10%, black 90%, transparent)",
            WebkitMaskImage: "linear-gradient(to right, transparent, black 10%, black 90%, transparent)",
        } : undefined}>

            {/* Scrolling track — duplicate content for seamless loop */}
            {[0, 1].map((i) => (<div key={i} className="marquee-track flex shrink-0 items-center" style={{
                gap: \`\${gap}px\`,
                paddingRight: \`\${gap}px\`,
                animation: \`marqueeScroll \${duration}s linear infinite\`,
                animationDirection: direction === "right" ? "reverse" : "normal",
...
function DefaultMarqueeItems() {
    ...
    return (<>
            {items.map((item, i) => (<div key={i} className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-medium text-white/80 whitespace-nowrap backdrop-blur-sm transition-colors hover:bg-white/10 hover:text-white">
                    <span className="h-2 w-2 rounded-full bg-indigo-400"/>
                    {item}
                </div>))}
        </>);
