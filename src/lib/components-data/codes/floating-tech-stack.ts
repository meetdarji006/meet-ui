...
export function FloatingTechStack({ techStacks = defaultTechStacks, radius = 170, className }) {
    ...
    return (<div ref={containerRef} className={cn(\`fts-\${uid} relative w-full h-[600px] flex items-center justify-center overflow-hidden bg-[#000000] perspective-[1500px]\`, className)} style={{
            backgroundImage: "radial-gradient(circle at 50% 50%, #151520 0%, #000000 80%)",
        }}>
            <style>{\`
                @keyframes vh-drop {
