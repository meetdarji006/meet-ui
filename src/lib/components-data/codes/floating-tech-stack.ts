// Auto-generated from floating-tech-stack.tsx
// Run: npm run generate-codes

export const floatingTechStackCodeTS = `"use client"
...
export function FloatingTechStack({ techStacks = defaultTechStacks, radius = 170, className }: FloatingTechStackProps) {
    ...
    const effectiveRadius = isMobile ? Math.min(radius, 110) : radius
    const centerSize = isMobile ? 120 : 160
    ...
    return (
        <div
            ref={containerRef}
            className={cn(
                \`fts-\${uid} relative w-full flex items-center justify-center overflow-hidden bg-[#000000] perspective-[1500px]\`,
                className
            )}
            style={{
        ...
        >
            ...
            <div ref={sceneRef} className={cn(\`vh-scene-\${uid} relative w-0 h-0\`)}>
                ...
                {/* Center Main Plate */}
                <SolidCard width={centerSize} height={centerSize} zOffset={0} delay={0} depth={isMobile ? 12 : 18} color="#6d28d9">
                    {/* Inner glowing logo box */}
                    <div className={cn("rounded-2xl bg-[#111118] border border-white/10 flex items-center justify-center shadow-[0_0_40px_rgba(109,40,217,0.5)]", isMobile ? "w-14 h-14" : "w-20 h-20")}>
...
export const floatingTechStackCodeJS = `"use client";
import React, { useId, useRef, useEffect, useState } from "react";
import { cn } from "@/lib/utils";
function SolidCard({ width, height, x = 0, y = 0, zOffset = 0, delay = 0, expandDelay = 1.6, color = "#8b5cf6", depth = 12, isSatellite = false, className, children, }) {
    const animationRules = isSatellite
        ? \`vh-drop 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards, vh-expand 1.0s cubic-bezier(0.16, 1, 0.3, 1) forwards, vh-float 5s ease-in-out infinite alternate\`
        : \`vh-float 5s ease-in-out infinite alternate\`;
    const delayRules = isSatellite
        ? \`\${delay}s, \${expandDelay}s, \${expandDelay + 1.0 + delay * 2}s\`
        : \`0s\`;
    return (<div className={cn("absolute rounded-2xl flex items-center justify-center", className)} style={{
            width,
            height,
            left: \`calc(50% - \${width / 2}px)\`,
            top: \`calc(50% - \${height / 2}px)\`,
            ...
            {/* 3D Extrusion Layers (Bottom to Top) */}
            {Array.from({ length: depth }).map((_, i) => (<div key={i} className="absolute inset-0 rounded-2xl pointer-events-none" style={{
                backgroundColor: color,
                transform: \`translateZ(-\${i + 1}px)\`,
                opacity: 1 - (i / depth) * 0.4,
                boxShadow: i === depth - 1 ? \`0 0 20px \${color}80\` : "none",
            ...
            {/* Top Surface */}
            <div className="absolute inset-0 z-10 rounded-2xl bg-[#09090b] flex items-center justify-center border border-white/20 shadow-[inset_0_1px_1px_rgba(255,255,255,0.15)] text-white/90 font-mono text-sm font-semibold tracking-wider backdrop-blur-sm" style={{ transform: "translateZ(0px)" }}>
                {children}
            </div>
        </div>);
}
...
export function FloatingTechStack({ techStacks = defaultTechStacks, radius = 170, className }) {
    const uid = useId().replace(/:/g, "");
    ...
    return (<div ref={containerRef} className={cn(\`fts-\${uid} relative w-full flex items-center justify-center overflow-hidden bg-[#000000] perspective-[1500px]\`, className)} style={{
            height: containerHeight,
            backgroundImage: "radial-gradient(circle at 50% 50%, #151520 0%, #000000 80%)",
        }}>
            <style>{\`
            ...
            <div ref={sceneRef} className={cn(\`vh-scene-\${uid} relative w-0 h-0\`)}>
                {/* Satellite Nodes */}
                {calculatedNodes.map((n) => (<SolidCard key={n.id} width={64} height={64} x={n.x} y={n.y} zOffset={0} delay={n.delay} expandDelay={expandDelay} depth={8} color="#4c1d95" isSatellite={true}>
                        {n.label}
                    </SolidCard>))}
                ...
                <SolidCard width={centerSize} height={centerSize} zOffset={0} delay={0} depth={isMobile ? 12 : 18} color="#6d28d9">
                    ...
                    <div className={cn("rounded-2xl bg-[#111118] border border-white/10 flex items-center justify-center shadow-[0_0_40px_rgba(109,40,217,0.5)]", isMobile ? "w-14 h-14" : "w-20 h-20")}>
                        {/* Dynamic lightning logo */}
                        <svg width={isMobile ? 28 : 40} height={isMobile ? 28 : 40} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M13 2L3 14H12L11 22L21 10H12L13 2Z" fill="#a855f7" stroke="#d8b4fe" strokeWidth="1.5" strokeLinejoin="round"/>
                        </svg>
                    </div>
                </SolidCard>
