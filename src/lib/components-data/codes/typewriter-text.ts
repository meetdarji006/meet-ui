// Auto-generated from typewriter-text.tsx
...
export function TypewriterText({ text, speed = 100, deleteSpeed = 50, waitTime = 2000, cursor = true, cursorChar = "|", loop = true, className = 'text-4xl md:text-5xl font-heading font-black tracking-tight', }) {
    const [displayText, setDisplayText] = useState("");
    const [isDeleting, setIsDeleting] = useState(false);
    const [loopNum, setLoopNum] = useState(0);
    const [typingSpeed, setTypingSpeed] = useState(speed);
    ...
    return (<div className={cn("inline-flex items-center", className)}>
            <span>{displayText}</span>
            {cursor && (<motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{
                duration: 0.5,
                repeat: Infinity,
                repeatType: "reverse",
            }} className={cn("ml-1", "inline-block")}>
