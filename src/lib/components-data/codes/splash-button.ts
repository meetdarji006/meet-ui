// Auto-generated from splash-button.tsx
// Run: npm run generate-codes

export const splashButtonCodeTS = `"use client"
...
export const splashButtonCodeJS = `"use client";
import React from "react";
import { cn } from "@/lib/utils";
export function SplashButton({ text = "Join Today", bgColor = "#7c3aed", accentColor = "#a78bfa", textColor = "#fff", className, onClick, }) {
    const uid = React.useId().replace(/:/g, "");
    const chars = text.split("");
    return (<>
            <style>{\`
            ...
            <button className={cn(\`sb-\${uid}\`, className)} onClick={onClick}>
                <div className="sb-bg"/>
                ...
                <div className="sb-wrap">
                    ...
                    <div className="sb-content">
                        {/* Text */}
                        <span className="sb-char">
                            {chars.map((ch, i) => (<span key={i} className={ch === " " ? "sb-space" : ""} style={{ "--i": i + 1 }}>
                                    {ch}
                                </span>))}
                        </span>
