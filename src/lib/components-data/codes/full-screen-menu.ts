// Auto-generated from full-screen-menu.tsx
// Run: npm run generate-codes

export const fullScreenMenuCodeTS = `import React, { useState, useEffect } from 'react';
...
export const fullScreenMenuCodeJS = `import React, { useState, useEffect } from 'react';
...
export function AnimatedMenuLink({ label, href, textColor }) {
    const [isHovered, setIsHovered] = useState(false);
    // Split text keeping spaces intact
    const chars = label.split('');
    return (<motion.li variants={navItemVariants} className="overflow-visible">
            <motion.a href={href} className="relative flex items-center group cursor-pointer text-5xl md:text-7xl font-sans font-medium uppercase tracking-tight leading-tight" style={{ color: textColor }} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} initial="initial" animate={isHovered ? "hover" : "initial"}>
                {/* Hover Dot Indicator */}
                <motion.div className="absolute -left-6 md:-left-8 w-2 md:w-3 h-2 rounded-full" style={{ backgroundColor: textColor }} initial={{ scale: 0, opacity: 0 }} animate={{ scale: isHovered ? 1 : 0, opacity: isHovered ? 1 : 0 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}/>

                {/* Rollover Letters */}
                <motion.div variants={wordContainerVariants} className="flex relative overflow-hidden">
                    {chars.map((char, index) => (<span key={index} className="relative inline-block whitespace-pre" style={{ perspective: '800px' }}>
                            {/* Original Character moving up/away */}
                            <motion.span variants={primaryCharVariants} className="inline-block origin-bottom">
                                {char}
                            </motion.span>

                            {/* Duplicate Character sliding up from below */}
                            <motion.span variants={secondaryCharVariants} className="absolute left-0 top-0 inline-block origin-top">
                                {char}
                            </motion.span>
                        </span>))}
                </motion.div>
...
export function FullScreenMenu({ className, links = defaultLinks, socials = defaultSocials, backgroundColor = '#000000', textColor = '#ffffff', lockBodyScroll = true }) {
    const [isOpen, setIsOpen] = useState(false);
    // Lock body scroll when menu is open
    useEffect(() => {
        if (!lockBodyScroll)
    ...
    return (<div className={cn("relative z-50", className)}>
            {/* Toggle Button */}
            <motion.button onClick={() => setIsOpen(!isOpen)} className="fixed top-6 right-6 md:top-8 md:right-10 z-60 flex items-center gap-2 px-4 py-2 bg-neutral-900/40 backdrop-blur-md text-white border border-white/10 rounded-full hover:bg-neutral-800/60 transition-colors" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                {isOpen ? <X size={18}/> : <Menu size={18}/>}
                <span className="text-sm font-medium tracking-wide">{isOpen ? 'CLOSE' : 'MENU'}</span>
            </motion.button>
            ...
            <AnimatePresence>
                {isOpen && (<>
                        {/* Optional dark backdrop overlay */}
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsOpen(false)} className="fixed inset-0 bg-black/20 backdrop-blur-sm z-40"/>

                        <motion.div variants={menuPanelVariants} initial="initial" animate="animate" exit="exit" className="fixed top-0 right-0 h-dvh w-full md:w-[45vw] lg:w-[35vw] z-50 shadow-2xl flex flex-col justify-center" style={{ backgroundColor }}>
                            {/* Navigation Links */}
                            <motion.ul variants={navListVariants} initial="initial" animate="animate" exit="exit" className="flex flex-col space-y-2 px-12 md:px-24">
                                {links.map((link, i) => (<AnimatedMenuLink key={i} label={link.label} href={link.href} textColor={textColor}/>))}
                            </motion.ul>

                            {/* Footer Socials */}
                            <motion.div variants={footerVariants} initial="initial" animate="animate" exit="exit" className="absolute bottom-10 left-12 md:left-24 text-xs tracking-widest flex flex-wrap gap-4 uppercase font-sans" style={{ color: \`\${textColor}99\` }} // Slightly faded text color
        >
                                <span className="font-semibold" style={{ color: textColor }}>SOCIALS:</span>
                                {socials.map((social, i) => (<a key={i} href="#" className="hover:text-white transition-colors">
                                        {social}
                                        {i !== socials.length - 1 && <span className="ml-4 opacity-50">,</span>}
                                    </a>))}
                            </motion.div>
