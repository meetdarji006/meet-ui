// Auto-generated from shooting-star.tsx
// Run: npm run generate-codes

export const shootingStarCodeTS = `"use client";
...
export const shootingStarCodeJS = `"use client";
import React, { useEffect, useRef } from "react";
import * as THREE from "three";
import { cn } from "@/lib/utils";
// --- SHADERS ---
...
export function ShootingStar({ className, starColor = "#aa8558", // Roughly vec3(170, 133, 88)
// Options
size = 0.05, minSize = 1, speed = 0.002, fadeSpeed = 1.1, shortRangeFadeSpeed = 1.3, minFlashingSpeed = 0.1, spread = 7, maxSpread = 5, maxZ = 100, blur = 1, far = 10, maxDiff = 100, diffPow = 0.24, }) {
    ...
    return (<div ref={containerRef} className={cn("fixed inset-0 pointer-events-none z-[-1] overflow-hidden", className)}>
            <canvas ref={canvasRef} className="block w-full h-full"/>
        </div>);
}
`
