// Auto-generated from ring-cursor.tsx
// Run: npm run generate-codes

export const ringCursorCodeTS = `"use client"
'use client';
...
export function RingCursor({
    ...
    }, [isVisible, dotSpringX, dotSpringY, ringSpringX, ringSpringY, isMobile]);
...
export const ringCursorCodeJS = `"use client";
'use client';
import React, { useEffect, useState } from 'react';
import { motion, useSpring } from 'framer-motion';
import { cn } from '@/lib/utils';
export function RingCursor({ className, color = '#ffffff', dotSize = 6, ringSize = 32, ringThickness = 1.5, ringStiffness = 200, ringDamping = 20, ringMass = 0.5, dotStiffness = 800, dotDamping = 35, dotMass = 0.5, }) {
    const [isVisible, setIsVisible] = useState(false);
    const [isHovering, setIsHovering] = useState(false);
    // Spring configurations
    const dotSpringConfig = { damping: dotDamping, stiffness: dotStiffness, mass: dotMass };
    ...
    if (isMobile)
        return null;
    return (<div className={cn("fixed inset-0 pointer-events-none z-9999 overflow-hidden transition-opacity duration-300", isVisible ? "opacity-100" : "opacity-0", className)}>
            {/* Outer Lagging Ring */}
            <motion.div className="absolute top-0 left-0 rounded-full border-solid flex items-center justify-center pointer-events-none" style={{
            x: ringSpringX,
            y: ringSpringY,
            // Centers the ring around the mouse coordinates
            translateX: '-50%',
            ...
            {/* Inner Spring Dot */}
            <motion.div className="absolute top-0 left-0 rounded-full pointer-events-none transition-opacity duration-300" style={{
            x: dotSpringX,
            y: dotSpringY,
            translateX: '-50%',
            translateY: '-50%',
