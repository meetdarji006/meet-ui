// Auto-generated from floating-tilt-tags.tsx
// Run: npm run generate-codes

export const floatingTiltTagsCodeTS = `"use client"
...
export const floatingTiltTagsCodeJS = `"use client";
import React, { useRef, useState, useEffect } from "react";
import { motion, useMotionValue, useSpring, useTransform } from "framer-motion";
import { cn } from "@/lib/utils";
export function FloatingTiltTags({ tags, className, tiltMagnitude = 20 // Max degrees of rotation
 }) {
    ...
    return (<div ref={containerRef} className={cn("relative flex items-center justify-center overflow-hidden perspective-distant", className)} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <motion.div className="relative w-full h-full flex items-center justify-center preserve-3d" style={{
            rotateX,
            rotateY,
            // transformStyle needed for children translateZ to work
            transformStyle: "preserve-3d"
            ...
            // Tags higher in the array sit "higher" physically on the Z axis.
            const dynamicZ = tag.z !== undefined ? tag.z : (index * 15);
            return (<div key={tag.id} className="absolute" style={{
                    left: tag.x,
                    top: tag.y,
                    transform: \`translate(-50%, -50%) translateZ(\${dynamicZ}px) rotate(\${tag.rotate || 0}deg)\`,
                }}>
                            <motion.div animate={{ y: index % 2 === 0 ? ["-4px", "4px"] : ["4px", "-4px"] }} transition={{
                    duration: 3 + (index % 3) * 0.5,
                    repeat: Infinity,
                    repeatType: "reverse",
                    ease: "easeInOut"
                }} className={cn("font-semibold tracking-tight px-4 py-2 text-2xl md:text-5xl shadow-lg select-none whitespace-nowrap", !tag.bgColor && !tag.color && (isDark
                    ? "bg-[#2A2929] text-[#FBF3D4] border border-[#FBF3D4]/10"
                    : "bg-[#FBF3D4] text-[#2A2929] border border-[#2A2929]/10"))} style={{
                    ...(tag.bgColor ? { backgroundColor: tag.bgColor } : {}),
                    ...(tag.color ? { color: tag.color } : {}),
                }}>
                                {tag.text}
