// Auto-generated from orbit-ecosystem.tsx
// Run: npm run generate-codes

export const orbitEcosystemCodeTS = `"use client";
...
export const orbitEcosystemCodeJS = `"use client";
import React, { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import { Layers, Box, Type, MousePointer2, Sparkles, Code2, LayoutTemplate, Palette, Zap, Terminal, Database, Frame } from 'lucide-react';
export const defaultOrbitItems = [
    { icon: Layers, label: 'Glassmorphism', color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
    { icon: Box, label: '3D Elements', color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/20' },
    { icon: Type, label: 'Typography', color: 'text-indigo-400', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20' },
    { icon: MousePointer2, label: 'Interactions', color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' },
...
export function OrbitEcosystem({ items = defaultOrbitItems, radiusInner = 120, radiusMiddle = 200, radiusOuter = 280, durationInner = 30, durationMiddle = 45, durationOuter = 60, reverseMiddle = true, }) {
    const [scale, setScale] = useState(1);
    useEffect(() => {
        const updateScale = () => {
            const width = window.innerWidth;
    ...
    // Calculate maximum bounding size based on the largest radius + padding for floating icons
    const maxDimensions = radiusOuter * 2 + 120;
    return (<div className="relative w-full flex flex-col items-center justify-center overflow-hidden py-10">
            {/* The scaled viewport bounding box that shrinks flow layout to prevent massive whitespace */}
            <div className="relative flex items-center justify-center pointer-events-auto transition-all duration-300" style={{ width: maxDimensions * scale, height: maxDimensions * scale }}>
                {/* The actual fixed-pixel ecosystem, scaled visually via CSS transform.
            Because the outer box shrinks, it flows perfectly with surrounding content! */}
            ...
            transformOrigin: 'center center'
                    ...
                    {/* Ring 1 (Inner) */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/5 border-dashed" style={{ width: radiusInner * 2, height: radiusInner * 2 }}/>

                    {/* Ring 2 (Middle) */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10 border-dashed" style={{ width: radiusMiddle * 2, height: radiusMiddle * 2 }}/>

                    {/* Ring 3 (Outer) */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-indigo-500/20 border-dashed" style={{ width: radiusOuter * 2, height: radiusOuter * 2 }}/>
            ...
            const isReversed = ringIndex === 1 ? reverseMiddle : false;
            const baseAngle = (positionInRing / 4) * 360;
            return (<div key={index} className="absolute left-1/2 top-1/2 pointer-events-none" style={{ transform: \`rotate(\${baseAngle}deg)\` }}>
                                <motion.div className="absolute left-0 top-0" initial={{ rotate: 0 }} animate={{ rotate: isReversed ? -360 : 360 }} transition={{
                    duration: duration,
                    repeat: Infinity,
                    ease: "linear",
                }}>
                                    <div className="absolute left-0 top-0 pointer-events-auto" style={{ transform: \`translateX(\${ringRadius}px)\` }}>
                                        {/* Counter-rotate the icon itself so it stays upright while orbiting */}
                                        <motion.div initial={{ rotate: -baseAngle }} animate={{ rotate: isReversed ? (-baseAngle + 360) : (-baseAngle - 360) }} transition={{
                    duration: duration,
                    repeat: Infinity,
                    ease: "linear",
                }} className={\`absolute -left-7 -top-7 group flex items-center justify-center w-14 h-14 rounded-2xl \${item.bg} \${item.border} border backdrop-blur-md cursor-pointer hover:scale-110 transition-transform duration-300 shadow-xl\`}>
