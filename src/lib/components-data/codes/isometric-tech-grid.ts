// Auto-generated from isometric-tech-grid.tsx
// Run: npm run generate-codes

export const isometricTechGridCodeTS = `"use client"
...
export function IsometricTechGrid({
    ...
    return (
        <div
            ref={containerRef}
            className={cn("relative w-full flex items-center justify-center overflow-hidden bg-[#0A0A0F] perspective-[2000px]", className)}
        ...
        >
            ...
            <div
                ...
                style={{
                    transformStyle: 'preserve-3d',
                    transform: \`\${isMobile ? \`scale(\${mobileScale})\` : ''} rotateX(\${mouseRotation.x}deg) rotateY(\${mouseRotation.y}deg) rotateZ(\${mouseRotation.z}deg)\`,
                }}
            >
                {/* Floor Grid Lines */}
                <div
...
export const isometricTechGridCodeJS = `"use client";
...
const defaultItems = [
    ...
    },
    {
        id: "react",
        x: -1, y: 0,
        icon: (<svg width="32" height="32" viewBox="-11.5 -10.23174 23 20.46348" xmlns="http://www.w3.org/2000/svg">
                <circle cx="0" cy="0" r="2.05" fill="#3B3B4F"/>
                <g stroke="#3B3B4F" strokeWidth="1" fill="none">
                    <ellipse rx="11" ry="4.2"/>
                    <ellipse rx="11" ry="4.2" transform="rotate(60)"/>
    ...
    },
    {
        ...
        x: 0, y: 0,
        isCenter: true,
        icon: (<div className="bg-white w-20 h-20 rounded shadow-md flex items-end justify-end p-2 relative">
                <span className="text-black font-bold text-4xl leading-none absolute bottom-1 right-2">JS</span>
            </div>)
    },
    {
...
function BaseNode({ x, y, spacing, children, isCenter, text }) {
    // Top-left coordinate relative to center 0,0
    const left = x * spacing;
    const top = y * spacing;
    const size = isCenter ? 140 : 120;
    return (<div className="absolute flex items-center justify-center transition-all duration-700 ease-out" style={{
            width: size,
            height: size,
            transform: \`translate3d(calc(\${left}px - 50%), calc(\${top}px - 50%), \${isCenter ? '40px' : '20px'})\`,
            transformStyle: "preserve-3d",
            ...
            {/* Dashed Drop Line down to the floor */}
            <div className="absolute top-1/2 left-1/2 w-0 border-l border-dashed border-white/20 pointer-events-none" style={{
            height: isCenter ? '200px' : '120px',
            transformOrigin: 'top',
            transform: 'translate(-50%, 0) rotateX(-90deg)', // Drop straight down in local Z
        }}/>
        ...
        <>
                    ...
                    {/* Purple glowing layers */}
                    {Array.from({ length: 6 }).map((_, i) => (<div key={i} className="absolute inset-0 rounded-3xl opacity-60 pointer-events-none" style={{
                    backgroundColor: '#a855f7',
                    transform: \`translateZ(\${-15 + (i * 2)}px)\`,
                    boxShadow: i === 0 ? '0 0 40px #a855f7, 0 0 80px #9333ea' : 'none',
                    filter: i === 0 ? 'blur(2px)' : 'none'
                    ...
                    <div className="absolute inset-0 bg-linear-to-br from-[#1a1a24] to-[#0f0f15] rounded-3xl border border-white/10 flex items-center justify-center text-white font-bold backdrop-blur-md shadow-2xl" style={{ transform: 'translateZ(0px)' }}>
                        {children || (text && (<div className="bg-white w-20 h-20 rounded shadow-md flex items-end justify-end p-2 relative">
                                <span className="text-black font-bold text-4xl leading-none absolute bottom-1 right-2">{text}</span>
                            </div>)) || null}
                    </div>
                </>) : (
        ...
        <>
                    <div className="absolute inset-0 bg-[#151520] rounded-3xl" style={{ transform: 'translateZ(-4px)' }}/>
                    <div className="absolute inset-0 bg-[#151520] rounded-3xl border border-white/5 flex items-center justify-center text-white/80 font-bold hover:bg-[#1a1a24] transition-colors cursor-pointer" style={{ transform: 'translateZ(0px)' }}>
                        {children || (text && <span className="text-xl font-bold text-white/50">{text}</span>)}
                    </div>
                </>)}
        </div>);
}
export function IsometricTechGrid({ className, items = defaultItems, spacing = 160, gridSize = 7 // 7x7 base floor grid
 }) {
    ...
    return (<div ref={containerRef} className={cn("relative w-full flex items-center justify-center overflow-hidden bg-[#0A0A0F] perspective-[2000px]", className)} style={{ height: isMobile ? 400 : 600 }}>
            {/* The 3D Scene Container */}
            <div className="relative w-0 h-0 transition-transform duration-200 ease-out" style={{
            transformStyle: 'preserve-3d',
            transform: \`\${isMobile ? \`scale(\${mobileScale})\` : ''} rotateX(\${mouseRotation.x}deg) rotateY(\${mouseRotation.y}deg) rotateZ(\${mouseRotation.z}deg)\`,
        }}>
                {/* Floor Grid Lines */}
            ...
            backgroundPosition: 'center center'
                    ...
                    {/* Glowing footprint under center node */}
                    <div className="absolute top-1/2 left-1/2 w-48 h-48 bg-purple-500/20 blur-[50px] rounded-full" style={{ transform: 'translate(-50%, -50%)' }}/>
                </div>

                {/* Render Nodes */}
                {positionedItems.map((item, idx) => (<BaseNode key={item.id ?? idx} x={item.x} y={item.y} spacing={spacing} isCenter={item.isCenter} text={item.text}>
                        {item.icon}
                    </BaseNode>))}
            </div>
        </div>);
