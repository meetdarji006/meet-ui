// Auto-generated from looping-words.tsx
...
import { motion, useMotionValue, useSpring, useTransform, animate } from "framer-motion";
import { cn } from "@/lib/utils";
export default function LoopingWords({ words = ["Design", "Develop", "Execute", "Succeed"], className, selectorColor = "white", activeColor = "white", inactiveColor = "rgba(255,255,255,0.4)", duration = 1.2, interval = 1.5, }) {
    const loopedWords = [...words, ...words, ...words];
    ...
    return (<div className={cn("relative w-full h-[2.7em] px-[0.1em] text-[10vw] md:text-[8vw] leading-[0.9] font-bold select-none font-heading", className)}>
            {/* Word List Container */}
            <div className="relative w-full h-full overflow-hidden">
                <motion.ul ref={listRef} className="relative flex flex-col items-center m-0 p-0 list-none" style={{ y: yTransform }}>
                    {loopedWords.map((word, i) => (<li key={i} className="flex items-center justify-center text-center uppercase whitespace-nowrap transition-colors duration-500" style={{ color: i % words.length === activeIndex % words.length ? activeColor : inactiveColor }}>
                            <p className="m-0">{word}</p>
                        </li>))}
                </motion.ul>
            </div>
            ...
            {/* Selector Brackets */}
            <motion.div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[0.9em] pointer-events-none z-20" style={{ width: selectorWidthSpring, color: selectorColor }}>
                <div className="absolute top-0 left-0 w-[0.125em] h-[0.125em] border-t-[0.035em] border-l-[0.035em] border-current"/>
                <div className="absolute top-0 right-0 w-[0.125em] h-[0.125em] border-t-[0.035em] border-r-[0.035em] border-current"/>
                <div className="absolute bottom-0 left-0 w-[0.125em] h-[0.125em] border-b-[0.035em] border-l-[0.035em] border-current"/>
                <div className="absolute bottom-0 right-0 w-[0.125em] h-[0.125em] border-b-[0.035em] border-r-[0.035em] border-current"/>
