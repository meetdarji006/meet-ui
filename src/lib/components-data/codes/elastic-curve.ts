// Auto-generated from elastic-curve.tsx
// Run: npm run generate-codes

export const elasticCurveCodeTS = `"use client"
...
export const elasticCurveCodeJS = `"use client";
import React, { useRef, useState, useCallback, useEffect } from "react";
import { motion, useSpring, useTransform } from "framer-motion";
export default function ElasticCurve({ className, height = 400, strokeWidth = 2, hitAreaWidth = 60, gradientStart = "rgb(255, 135, 9)", gradientEnd = "rgb(247, 189, 248)", stiffness = 400, damping = 12 }) {
    ...
    return (<div className={className} style={{ width: '100%', height }}>
            <svg ref={svgRef} viewBox={\`0 0 800 \${height}\`} style={{ width: '100%', height: '100%' }} preserveAspectRatio="none">
                <defs>
                    <linearGradient id={gradientId} x1="0" y1="0" x2="596" y2="600" gradientUnits="userSpaceOnUse">
                        <stop offset="0.2" stopColor={gradientStart}/>
                        <stop offset="0.7" stopColor={gradientEnd}/>
                    </linearGradient>
                </defs>
                ...
                {/* Visible curved line */}
                <motion.path d={pathD} stroke={\`url(#\${gradientId})\`} strokeWidth={strokeWidth} fill="none" strokeLinecap="round"/>

                {/* Invisible hit area for hover detection */}
                <path d={pathD} stroke="transparent" strokeWidth={hitAreaWidth} fill="none" style={{ cursor: 'pointer' }} onPointerMove={handlePointerMove} onPointerLeave={handlePointerLeave}/>
            </svg>
        </div>);
}
`
