// Auto-generated from stacked-info-cards.tsx
// Run: npm run generate-codes

export const stackedInfoCardsCodeTS = `"use client"
...
export const stackedInfoCardsCodeJS = `"use client";
import React, { useState, useCallback, useEffect } from "react";
import { cn } from "@/lib/utils";
import { motion, AnimatePresence } from "framer-motion";
import { ArrowLeft, ArrowRight } from "lucide-react";
export function StackedInfoCards({ items, className, cardColor = "#7c3aed", stackColor = "#a78bfa", textColor = "#ffffff", navColor = "#ffffff", navActiveColor = "#7c3aed", borderRadius = 20, showDashedLine = true, }) {
    const [currentIndex, setCurrentIndex] = useState(0);
    const [direction, setDirection] = useState(0);
    ...
    const leftStack = getLeftStack();
    const rightStack = getRightStack();
    return (<div className={cn("relative w-full px-8 sm:px-10 overflow-hidden sm:overflow-visible", className)}>
            <div className="relative w-full mx-auto" style={{ minHeight: "clamp(340px, 65vw, 480px)", maxWidth: 520 }}>
                {/* Left stack — previous cards */}
                {leftStack.map(({ index, depth }) => (<motion.div key={\`left-\${index}\`} className="absolute inset-0" style={{
                backgroundColor: stackColor,
                borderRadius,
                zIndex: 5 - depth,
            }} animate={{
                ...
                {/* Right stack — upcoming cards */}
                {rightStack.map(({ index, depth }) => (<motion.div key={\`right-\${index}\`} className="absolute inset-0" style={{
                backgroundColor: stackColor,
                borderRadius,
                zIndex: 5 - depth,
            }} animate={{
                ...
                {/* Active card */}
                <AnimatePresence initial={false} custom={direction} mode="popLayout">
                    <motion.div key={currentIndex} custom={direction} className="absolute inset-0" style={{
            backgroundColor: cardColor,
            borderRadius,
            color: textColor,
            zIndex: 10,
            ...
            mass: 0.9,
                        ...
                        <div className="p-5 sm:p-8 md:p-10 flex flex-col h-full" style={{ minHeight: "clamp(340px, 65vw, 480px)" }}>
                            <div className="mb-2">
                                <span className="text-xs font-bold uppercase tracking-[0.25em]" style={{ color: textColor }}>
                                    {items[currentIndex].label}
                                </span>
                            </div>

                            {showDashedLine && (<div className="w-full mb-6" style={{
                borderTop: \`2px dashed \${textColor}\`,
                opacity: 0.4,
            }}/>)}

                            <h3 className="text-xl sm:text-2xl md:text-3xl font-light leading-snug mb-3 sm:mb-4" style={{ color: textColor, fontFamily: "serif" }}>
                                {items[currentIndex].title}
                            </h3>

                            <p className="text-sm sm:text-base md:text-lg leading-relaxed opacity-90" style={{ color: textColor }}>
                                {items[currentIndex].description}
                            </p>
                        </div>
                    </motion.div>
                ...
                {/* Navigation arrows */}
                <div className="absolute bottom-4 sm:bottom-6 left-5 sm:left-8 z-20 flex items-center gap-2">
                    <button onClick={() => paginate(-1)} className="w-9 h-9 sm:w-11 sm:h-11 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-95" style={{
            backgroundColor: "#fff",
            color: cardColor,
        }} aria-label="Previous">
                        <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5"/>
