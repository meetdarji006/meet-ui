// Auto-generated from hero-text-hover.tsx
// Run: npm run generate-codes

export const heroTextHoverCodeTS = `"use client"
...
export const heroTextHoverCodeJS = `"use client";
import React, { useState, useRef, useEffect } from "react";
import { motion, useMotionValue, useSpring, useMotionTemplate, useTransform } from "framer-motion";
import { cn } from "@/lib/utils";
export const HeroTextHover = ({ baseText, revealText, baseLayerClassName, revealLayerClassName, className, }) => {
    ...
    return (<div ref={containerRef} className={cn("relative flex items-center justify-center overflow-hidden bg-black py-20 px-4", className)} onMouseMove={handleMouseMove} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            {/* Base Layer (Bottom) */}
            <div className={cn("w-full h-full flex flex-col items-center justify-center text-center text-8xl md:text-[12rem] leading-[0.7] font-black tracking-tighter text-zinc-800 select-none uppercase", baseLayerClassName)}>
                {renderText(baseText)}
            </div>
