// Auto-generated from click-ripple.tsx
// Run: npm run generate-codes

export const clickRippleCodeTS = `"use client"
...
export const clickRippleCodeJS = `"use client";
import { useEffect, useState, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
export const ClickRipple = ({ rippleSize = 400, duration = 800, color = "#667eea", enabled = true, }) => {
    ...
    return (<div style={{
            ...
            <AnimatePresence>
                {ripples.map((ripple) => (<motion.div key={ripple.id} initial={{
                scale: 0,
                opacity: 0.6,
            }} animate={{
                scale: 1,
