...
> [!NOTE]
> The dynamic loader automatically finds the export ending with `Registry` (e.g., `myComponentRegistry`) in your file. Ensure your registry object follows this naming convention.

---

## Step 5: Add to Codes Index

The `npm run build` command (or `npm run generate-codes`) automatically generates the code strings in `src/lib/components-data/codes/`. You don't need to manually create these files. Just ensure your component is in the registry.
